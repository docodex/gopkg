...
func TestGenerate(t *testing.T) {
	...
	sleepTime := int64(50)
	time.Sleep(time.Duration(sleepTime * 10 * int64(time.Millisecond)))
	...
	actualTime := (s.Timestamp(id) - now.UnixMilli()) / 10
...
func TestGenerate_InSequence(t *testing.T) {
	now := time.Now()
	s, err := snowflake.New(snowflake.WithEpoch(now), snowflake.WithTimeUnit(time.Millisecond))
	if err != nil {
		t.Fatalf("failed to create snowflake: %v", err)
	}
...
func TestGenerate_ReturnsError(t *testing.T) {
	s, err := snowflake.New(
		...
		snowflake.WithTimeUnit(time.Millisecond),
...
func BenchmarkGenerateMinTimeUnit(b *testing.B) {
	s, err := snowflake.New(snowflake.WithTimeUnit(time.Millisecond), snowflake.WithNode(1))
	if err != nil {
