...
const (
	...
	// You may modify this constant to set a different epoch timestamp for your application.
	...
	// You may modify this constant to set a different time unit for your application.
	timeUnit = 1 // 1ms
	...
	// You may modify this constant to set a different length for your application.
...
func (s *Snowflake) Generate() (int64, error) {
	...
	if now == s.elapsed {
		...
		// Wait for the clock to catch up. Release s.mu during Sleep so other goroutines
		// calling Generate are not blocked for up to 2 seconds (maxTolerableRollback).
		// On each wake-up we re-check the rollback threshold because another goroutine
		// may have advanced s.elapsed while we were sleeping.
		for now <= s.elapsed {
			s.mu.Unlock()
...
// Node returns the node id configured for this Snowflake instance.
