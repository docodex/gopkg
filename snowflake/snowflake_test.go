...
func TestNew(t *testing.T) {
	...
	// 1<<10 - 1 is the maximum valid node id; 1<<10 must be rejected.
	if _, err = snowflake.New(snowflake.WithNode(1<<10 - 1)); err != nil {
		t.Fatalf("unexpected error at max node boundary: %v", err)
	}
	if _, err = snowflake.New(snowflake.WithNode(1 << 10)); !errors.Is(err, snowflake.ErrOverNodeLimit) {
		t.Fatalf("expected ErrOverNodeLimit, got: %v", err)
	}
	if _, err = snowflake.New(snowflake.WithNode(-1)); !errors.Is(err, snowflake.ErrOverNodeLimit) {
		t.Fatalf("expected ErrOverNodeLimit, got: %v", err)
	}
}
...
func TestGenerate(t *testing.T) {
	...
	if actualTime < sleepTime || actualTime > sleepTime+20 {
