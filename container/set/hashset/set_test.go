...
func TestSet_AliasedOperations(t *testing.T) {
	// dst aliases a src in Union/Intersection/Difference must not corrupt src.
	a := hashset.New(1, 2, 3)
	b := hashset.New(2, 3, 4)

	set.Union(a, a, b)
	assert.Equal(t, 4, a.Len())
	assert.True(t, a.Contains(1, 2, 3, 4))

	a = hashset.New(1, 2, 3)
	set.Intersection(a, a, b)
	assert.Equal(t, 2, a.Len())
	assert.True(t, a.Contains(2, 3))
...
func TestSet_WithLock(t *testing.T) {
	s := hashset.NewWithLock(1, 2, 3, 4, 5)
