...
// TestMoveCorrectness exercises the walk-and-detect logic with both small and
// large |n| across forward and backward directions, including n that exceeds
// the ring size (requiring modulo reduction after one full lap).
func TestMoveCorrectness(t *testing.T) {
	...
	cases := []struct {
		...
		{7, 2},    // 7 mod 5 == 2
		{10, 0},   // 2 laps
		{-1, 4},   // walk back from 0
		{-5, 0},   // full lap backward
		{-6, 4},   // -6 mod 5 normalized
		{1000, 0}, // 1000 mod 5 == 0
		{-1000, 0},
	}
	for _, tc := range cases {
		got := r.Move(tc.n).Value
		if got != tc.want {
			t.Errorf("Move(%d).Value = %d, want %d", tc.n, got, tc.want)
		}
	}
}
