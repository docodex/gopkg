...
//
// The complexity is O(min(|n|, r.Len())): Move walks in the chosen direction
// and also detects a full lap by returning to the start. Neither a small n
// on a large ring nor a huge |n| on a small ring pays more than O(size).
func (r *Ring[T]) Move(n int) *Ring[T] {
	// Walk-and-detect: step in the chosen direction; if we come back to the
	// starting node before consuming n, we have measured the ring size and
	// can reduce n modulo size to finish. This avoids an eager O(size) Len()
	...
	case n < 0:
		...
		for i := 0; i > n; i-- {
			r = r.prev
			if r == start {
				// Competed a full lap at step (-i + 1); i is 0, -1, -2, ...
				size := -i + 1
				n %= size
				for ; n < 0; n++ {
					r = r.prev
	...
	case n > 0:
		...
		for i := 0; i < n; i++ {
			r = r.next
			if r == start {
				// Completed a full lap at step i+1, so size == i+1.
				size := i + 1
				n %= size
				for ; n > 0; n-- {
					r = r.next
