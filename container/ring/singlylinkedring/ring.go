...
type Ring[T any] struct {
	next  *Ring[T] // Pointer to next ring element.
...
//
// For n >= 0 the complexity is O(min(n, r.Len())): the walk stops early if
// n is smaller than the ring. For n < 0 the complexity is O(r.Len()), since
// a singly-linked ring cannot walk backward and must first learn the size
// to translate a backward move into a forward one.
func (r *Ring[T]) Move(n int) *Ring[T] {
	...
	if n < 0 {
		// Singly-linked: must learn size to translate backward into forward.
		size := r.Len()
		n = (n%size + size) % size
	}
	// Walk forward: if we complete a lap before reaching n, collapse via modulo.
	// (For the n<0 path, n is already in [0, size), so the lap check never fires.)
	start := r
	for i := 0; i < n; i++ {
		r = r.next
		if r == start {
...
func (r *Ring[T]) String() string {
	...
	return "SinglyLinkedRing: " + values
}
