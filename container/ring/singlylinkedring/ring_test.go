package singlylinkedring_test

import (
	"fmt"
	"testing"

	"github.com/docodex/gopkg/container/ring/singlylinkedring"
)

func TestNewAndAdd(t *testing.T) {
	r := singlylinkedring.New(1, 2, 3, 4)
...
func dump[T any](r *singlylinkedring.Ring[T]) {
	if r == nil {
		fmt.Println("empty")
		return
	}
...
func verify(t *testing.T, r *singlylinkedring.Ring[int], N int, sum int) {
	if r == nil {
		return
	}
	...
	// connections
	if r.Next() != nil {
		var x *singlylinkedring.Ring[int] // previous element
...
func TestCornerCases(t *testing.T) {
	var (
		r0 *singlylinkedring.Ring[int]
		r1 singlylinkedring.Ring[int]
	)
	// Basics
	verify(t, r0, 0, 0)
	verify(t, &r1, 1, 0)
...
func newDefault(t *testing.T, n int) *singlylinkedring.Ring[int] {
	if n <= 0 {
		return nil
	}
	r := singlylinkedring.New(0)
	for i := 1; i < n; i++ {
		r.Add(0)
	}
	size := r.Len()
...
func makeN(t *testing.T, n int) *singlylinkedring.Ring[int] {
	r := newDefault(t, n)
	for i := 1; i <= n; i++ {
		r.Value = i
		r = r.Next()
...
func TestLink1(t *testing.T) {
	r1a := makeN(t, 1)
	var r1b singlylinkedring.Ring[int]
	r2a := r1a.Link(&r1b)
	verify(t, r2a, 2, 1)
	if r2a != r1a {
...
func TestLink2(t *testing.T) {
	var r0 *singlylinkedring.Ring[int]
	r1a := &singlylinkedring.Ring[int]{Value: 42}
	r1b := &singlylinkedring.Ring[int]{Value: 77}
	r10 := makeN(t, 10)
...
func TestLink3(t *testing.T) {
	var r singlylinkedring.Ring[int]
	n := 1
	for i := 1; i < 10; i++ {
		n += i
...
func TestDeleteRing(t *testing.T) {
	for i := range 5 {
		...
		buf := make([]*singlylinkedring.Ring[int], 0, i)
		buf = append(buf, r)
