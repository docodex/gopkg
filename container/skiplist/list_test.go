...
// assertRankInvariant checks that after bulk removals the remaining elements
// report a 1-based rank matching their ordered position. This validates that
// RemoveRange maintains span bookkeeping across levels, since Get's rank is
// computed by walking the skip-level spans.
func assertRankInvariant(t *testing.T, l *skiplist.Skiplist[int, int]) {
	t.Helper()
	keys := l.Keys()
	...
	if len(keys) != l.Len() {
		t.Fatalf("Len()=%d but Keys() returned %d items", l.Len(), len(keys))
	}
	for i, k := range keys {
		e, rank := l.Get(k)
		if e == nil {
			t.Fatalf("Get(%d) returned nil element", k)
		}
		if rank != i+1 {
			t.Fatalf("Get(%d) rank=%d, want %d", k, rank, i+1)
		}
	}
}

func TestSkiplistRemoveRange_MultiLevelInvariant(t *testing.T) {
	// Build a skiplist large enough to have multiple levels, delete a
	// contiguous middle range, and verify that the remaining elements
	// still expose correct ranks via Get. Run the scenario repeatedly
	// so randomness of level choice is exercised.
	const n = 200
	for trial := range 20 {
		l := skiplist.New[int, int]()
		for i := range n {
		...
		// Delete a wide middle range that spans many levels.
		removed := l.RemoveRange(50, 150)
		if len(removed) != 100 {
			t.Fatalf("trial %d: RemoveRange removed %d, want 100", trial, len(removed))
		}
		if l.Len() != n-100 {
			t.Fatalf("trial %d: Len=%d, want %d", trial, l.Len(), n-100)
		}
		assertRankInvariant(t, l)

		// Remove another disjoint range to stress repeated bulk removal.
		removed2 := l.RemoveRange(20, 40)
		if len(removed2) != 20 {
			t.Fatalf("trial %d: RemoveRange second call removed %d, want 20", trial, len(removed2))
		}
		assertRankInvariant(t, l)
	}
}

func TestSkiplistRemoveRangeByRank_MultiLevelInvariant(t *testing.T) {
	const n = 200
	for trial := range 20 {
		l := skiplist.New[int, int]()
		for i := range n {
		...
		// rank is 1-based; remove ranks [50, 150)
		removed := l.RemoveRangeByRank(50, 150)
		if len(removed) != 100 {
			t.Fatalf("trial %d: RemoveRangeByRank removed %d, want 100", trial, len(removed))
		}
		if l.Len() != n-100 {
			t.Fatalf("trial %d: Len=%d, want %d", trial, l.Len(), n-100)
		}
...
func TestSkiplistRemoveRange_Randomized(t *testing.T) {
	// Randomized delete-and-verify. Compare skiplist state against a sorted
	// reference slice; after each bulk removal, ranks must still match.
	...
	for trial := range 50 {
		lo := rng.IntN(10000)
		hi := lo + rng.IntN(2000) + 1
		removed := l.RemoveRange(lo, hi)
		count := 0
		for k := range ref {
			if k >= lo && k < hi {
				delete(ref, k)
		...
		if len(removed) != count {
			t.Fatalf("trial %d: removed=%d, want %d (range [%d,%d))", trial, len(removed), count, lo, hi)
		}
		if l.Len() != len(ref) {
			t.Fatalf("trial %d: Len=%d, want %d", trial, l.Len(), len(ref))
		}
		assertRankInvariant(t, l)
	}
}
