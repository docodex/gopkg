...
// NewWithLock returns an initialized bidirectional map with the default capacity as the initial
// capacity for the backing hash tables and a sync.RWMutex to support concurrent use by multiple
// goroutines.
func NewWithLock[K comparable, V comparable]() *Map[K, V] {
	return &Map[K, V]{
		forward: make(map[K]V, defaultCapacity),
		inverse: make(map[V]K, defaultCapacity),
		mu:      &sync.RWMutex{},
...
// NewWithCapacityAndLock returns an initialized bidirectional map with the given capacity as the
// initial capacity for the backing hash tables and a sync.RWMutex to support concurrent use by
// multiple goroutines.
func NewWithCapacityAndLock[K comparable, V comparable](capacity int) *Map[K, V] {
	capacity = max(capacity, defaultCapacity)
	return &Map[K, V]{
		forward: make(map[K]V, capacity),
		inverse: make(map[V]K, capacity),
...
// ErrDuplicateValue is returned by UnmarshalJSON when the input has two or
// more keys mapping to the same value, which would violate the bidirectional
// map's one-to-one invariant.
var ErrDuplicateValue = errors.New("hashbidimap: duplicate value in input breaks one-to-one invariant")

// UnmarshalJSON unmarshals a JSON description of map.
// The input can be assumed to be a valid encoding of a JSON value.
// UnmarshalJSON must copy the JSON data if it wishes to retain the data after
// returning.
// Ref: std json.Unmarshaler.
//
// If the input contains duplicate values, ErrDuplicateValue is returned and
// the map is left unchanged. The update is atomic: the forward/inverse maps
// are swapped in only after both are fully validated and built.
...
// Contains returns true if map contains all the given keys k.
...
// ContainsValues returns true if map contains all the given values v.
