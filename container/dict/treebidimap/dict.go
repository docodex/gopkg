...
// for the backing red-black trees and a sync.RWMutex to support concurrent use by multiple
// goroutines.
...
var ErrDuplicateValue = errors.New("treebidimap: duplicate value in input breaks one-to-one invariant")

// UnmarshalJSON unmarshals a JSON description of map.
// The input can be assumed to be a valid encoding of a JSON value.
// UnmarshalJSON must copy the JSON data if it wishes to retain the data
// after returning.
// Ref: std json.Unmarshaler.
//
// If the input contains duplicate values, ErrDuplicateValue is returned
// and the map is left unchanged. On any other failure the map is also left
// unchanged (the update is atomic: forward and inverse are swapped in
// together only after both parse successfully).
func (m *Map[K, V]) UnmarshalJSON(data []byte) error {
	...
	// Build both trees in locals first so any failure leaves m untouched.
	forward := redblacktree.NewFunc[K, V](m.forward.Compare())
	inverse := redblacktree.NewFunc[V, K](m.inverse.Compare())
	if err := forward.UnmarshalJSON(data); err != nil {
		return err
	}
	if err := inverse.UnmarshalJSON(inverseData); err != nil {
		return err
	}
	if m.mu != nil {
		m.mu.Lock()
