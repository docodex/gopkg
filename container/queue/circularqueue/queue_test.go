...
func TestQueueDequeueFull(t *testing.T) {
	...
	ok = q.Enqueue(3) // queue full; Enqueue is a no-op and returns false
	assert(ok, false)
	assert(q.Len(), 2)
...
// TestQueueWrapAround exercises the state where the internal buffer wraps:
// first > tail. After partial drain + refill, Values() must return elements
// in FIFO order regardless of which underlying slot they occupy.
func TestQueueWrapAround(t *testing.T) {
	...
	q.Enqueue(1)
	q.Enqueue(2)
	q.Enqueue(3) // tail wraps back to 0
	v, ok := q.Dequeue()
	assert(v, 1)
	assert(ok, true)
	ok = q.Enqueue(4) // lands at index 0 (wrap), first=1, tail=1 -> wrap state
	assert(ok, true)
	want := []int{2, 3, 4}
	got := q.Values()
	if len(got) != len(want) {
	...
	for i := range want {
		if got[i] != want[i] {
			t.Errorf("Values[%d]: got %d want %d", i, got[i], want[i])
		}
	}
	// Drain and confirm FIFO.
	for _, w := range want {
		g, ok := q.Dequeue()
		assert(ok, true)
		assert(g, w)
...
// TestUnmarshalJSONBoundaries covers len(v) == cap and len(v) > cap.
func TestUnmarshalJSONBoundaries(t *testing.T) {
	// len == cap: full queue, tail wraps to 0 (equal to first).
	q := circularqueue.New[int](3)
	if err := q.UnmarshalJSON([]byte(`[1,2,3]`)); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if q.Len() != 3 || !q.Full() {
		t.Error("expected full queue")
	}
	for i, want := range []int{1, 2, 3} {
		g, ok := q.Dequeue()
		if !ok || g != want {
			t.Errorf("Dequeue[%d]: got %v, %v want %d, true", i, g, ok, want)
		}
	}

	// len > cap: must return ErrOverCapacity and leave queue unchanged.
	q = circularqueue.New[int](2)
	q.Enqueue(9)
	err := q.UnmarshalJSON([]byte(`[1,2,3]`))
	if err != circularqueue.ErrOverCapacity {
		t.Errorf("expected ErrOverCapacity, got %v", err)
	}
	if q.Len() != 1 {
		t.Errorf("queue modified after failed UnmarshalJSON: len=%d", q.Len())
	}
	if v, _ := q.Peek(); v != 9 {
		t.Errorf("queue modified after failed UnmarshalJSON: peek=%d", v)
	}
}
...
func BenchmarkCircularQueueEnqueue100(b *testing.B) {
	b.StopTimer()
	size := 100
	queue := circularqueue.New[int](size)
	b.StartTimer()
