...
func TestQueueString(t *testing.T) {
	q := linkedlistqueue.New[any]()
	q.Enqueue(1)
	if !strings.HasPrefix(q.String(), "LinkedListQueue") {
		t.Errorf("String should start with container name")
	}
}

func benchmarkEnqueue(b *testing.B, q *linkedlistqueue.Queue[any], size int) {
	for b.Loop() {
		for n := range size {
			q.Enqueue(n)
		}
...
func benchmarkDequeue(b *testing.B, q *linkedlistqueue.Queue[any], size int) {
	for b.Loop() {
		for range size {
			q.Dequeue()
		}
