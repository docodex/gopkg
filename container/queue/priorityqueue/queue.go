...
func NewFunc[T any](less container.Less[T], v ...T) *Queue[T] {
	if less == nil {
		panic("priorityqueue: less function must not be nil")
	}
	q := &Queue[T]{
		elements: nil,
		less:     less,
...
// MarshalJSON marshals queue into valid JSON in Dequeue order, so the JSON
// representation is stable for any two queues that would dequeue identical
// value sequences, regardless of the internal heap-array layout.
...
func (q *Queue[T]) UnmarshalJSON(data []byte) error {
	if q.less == nil {
		return errors.New("priorityqueue: less function must be set (non-nil) before UnmarshalJSON")
	}
	var v []T
...
func (q *Queue[T]) Fix(i int) {
	if i < 0 || i >= len(q.elements) || q.elements[i].index != i {
		return
	}
	if !q.shiftDown(i) {
		q.shiftUp(i)
