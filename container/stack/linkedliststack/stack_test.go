...
func benchmarkPush(b *testing.B, s *linkedliststack.Stack[any], size int) {
	for b.Loop() {
		for n := range size {
			s.Push(n)
		}
...
func benchmarkPop(b *testing.B, s *linkedliststack.Stack[any], size int) {
	for b.Loop() {
		for range size {
			s.Pop()
		}
