...
// String returns the string representation of stack (in LIFO order).
...
// MarshalJSON marshals stack into valid JSON in Push order (bottom-to-top),
// matching linkedliststack so that the two implementations are JSON-interchangeable
// through the stack.Stack interface.
...
// The input is expected to be in Push order (as produced by MarshalJSON).
