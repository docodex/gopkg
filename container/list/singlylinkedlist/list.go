...
// removed node value.
//
// The removed node's Value is intentionally left intact so callers that still
// hold *Node references can read the value after removal, matching the
// doublylinkedlist convention.
...
func (l *List[T]) Clear() {
	...
	for x := l.root.next; x != &l.root; {
		...
		x.list = nil
		x.Value = zero // avoid memory leaks via held value payloads
		x = y
	}
	l.init()
}
...
//
// Because the list is singly linked, locating index i requires walking from the head, so Add
// runs in O(i + len(v)). Callers inserting at a position they already hold a *Node for should
// prefer [List.InsertAfter] for O(len(v)) insertion, and batches that add many values at
// successive indices should hold the returned *Node and chain InsertAfter calls instead of
// calling Add in a loop (which is O(n^2)).
...
//
// Because the list is singly linked, locating index i requires walking from the head, so Del
// runs in O(i). Callers that already hold a *Node for the predecessor should prefer
// [List.RemoveAfter] for O(1) removal; deleting at many successive indices in a loop is O(n^2)
// and should be rewritten to walk with [Node.Next].
...
//
// Sort is implemented by extracting all values, sorting them, clearing the list
// and reinserting them, so any *Node[T] pointers the caller previously obtained
// from this list are invalidated and must not be reused.
...
// RRange calls f sequentially (in reverse order) for each index i and value v
// present in list. If f returns false, range stops the iteration.
//
// Because List is singly linked, RRange materializes the values into a
// slice first, so the space cost is O(n). Use arraylist or doublylinkedlist
// if you need O(1) per-step reverse traversal.
