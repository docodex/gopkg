...
type List[T any] struct {
	root Node[T] // sentinel list node, only &root, root.prev, and root.next are used
...
// insert inserts new nodes with the given values v after at, increments l.len, and
// returns the first node just inserted.
...
// remove removes x from its list, decrements l.len, and returns the removed node value.
//
// The removed node's Value is intentionally left intact so callers that still
// hold *Node references (e.g. via Remove) can read the value after removal,
// matching the Go standard library container/list convention.
func (l *List[T]) remove(x *Node[T]) (value T, ok bool) {
	if x == &l.root {
		return
	}
	x.prev.next = x.next
...
func (l *List[T]) RRange(f func(i int, v T) bool) {
	...
	for i, x := l.len-1, l.root.prev; i >= 0; i, x = i-1, x.prev {
		if !f(i, x.Value) {
			break
		}
	}
...
// Remove removes x from l if x is a node of list l.
// It returns the node value x.Value.
func (l *List[T]) Remove(x *Node[T]) (value T, ok bool) {
	if x != nil && x.list == l && x != &l.root {
...
// MoveToFront moves node x to the front of list l.
...
// MoveToBack moves node x to the back of list l.
// If x is not a node of l, the list is not modified.
...
// MoveBefore moves node x to its new position before mark.
// If x or mark is not a node of l, or x == mark, the list is not modified.
func (l *List[T]) MoveBefore(x, mark *Node[T]) {
	if x == nil || x.list != l || mark == nil || mark.list != l {
		return
	}
	l.move(x, mark.prev)
}

// MoveAfter moves node x to its new position after mark.
// If x or mark is not a node of l, or x == mark, the list is not modified.
...
func (l *List[T]) PushFrontList(other *List[T]) {
	if other != nil {
		for i, x := other.Len(), other.BackNode(); i > 0; i, x = i-1, x.Prev() {
			l.insert(&l.root, x.Value)
		}
	}
}
