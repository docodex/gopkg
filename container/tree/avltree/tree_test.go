...
func TestAVLRemove(t *testing.T) {
	...
	t.Run("Random Test", func(t *testing.T) {
		...
		for _, n := range nums {
			...
			// Sample the in-order invariant instead of re-traversing on every Remove, which
			// made this O(n^2) and very slow under -race.
			step := max(n/20, 1)
			for i := range n {
				t1.Remove(nums[i])
				if i%step == 0 || i == n-1 {
...
// nodeHeight returns the height of the subtree rooted at x using only the
// public Left/Right accessors. Height of a nil node is -1, left is 0.
func nodeHeight[K comparable, V any](x *avltree.Node[K, V]) int {
	if x == nil {
		return -1
	}
	l := nodeHeight(x.Left())
	r := nodeHeight(x.Right())
	if l > r {
		return l + 1
	}
	return r + 1
...
func assertAVLInvariants[K comparable, V any](t *testing.T, x *avltree.Node[K, V], cmp func(a, b K) int) {
	t.Helper()
	if x == nil {
		return
	}
	bf := nodeHeight(x.Left()) - nodeHeight(x.Right())
	if bf < -1 || bf > 1 {
		t.Fatalf("AVL balance factor out of range at key=%v: bf=%d", x.Key(), bf)
	}
	if l := x.Left(); l != nil && cmp(l.Key(), x.Key()) >= 0 {
		t.Fatalf("AVL BST order violated: left key %v >= parent %v", l.Key(), x.Key())
	}
	if r := x.Right(); r != nil && cmp(r.Key(), x.Key()) <= 0 {
		t.Fatalf("AVL BST order violated: right key %v <= parent %v", r.Key(), x.Key())
	}
	assertAVLInvariants(t, x.Left(), cmp)
	assertAVLInvariants(t, x.Right(), cmp)
}
...
func TestAVLInvariants_AfterInsertsAndRemoves(t *testing.T) {
	...
	for _, n := range sizes {
		...
		for i, v := range nums {
			tr.Insert(v, struct{}{})
			if i%97 == 0 {
				assertAVLInvariants(t, tr.Root(), intCmp)
			}
		}
		assertAVLInvariants(t, tr.Root(), intCmp)
...
func TestAVLInvariants_Randomized(t *testing.T) {
