...
// Compare returns the comparator function used by tree t.
func (t *Tree[K, V]) Compare() container.Compare[K] {
	return t.cmp
}
...
func (t *Tree[K, V]) removeFixupLeft(p *Node[K, V]) *Node[K, V] {
	...
	if t.color(p.right.right) == black && t.color(p.right.left) == black {
		// sibling is black, and has two black children, while x (current node) is dual-black
		// consider moving a black color from sibling and x to parent p
		// then, parent p has an additional black color
		// so, p has dual-black or red-black color
