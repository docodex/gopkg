...
func TestRBTree(t *testing.T) {
	...
	for _, n := range testcases {
		t1 := redblacktree.New[int, struct{}]()
		nums := rand.Perm(n)
		for _, v := range nums {
			t1.Insert(v, struct{}{})
		}
...
func TestRBInit(t *testing.T) {
	...
	for _, n := range testcases {
		...
		assert.Nil(t, err)
		t2 := redblacktree.New[int, struct{}]()
...
func TestRBInsert(t *testing.T) {
	...
	t.Run("RRRotation-Test", func(t *testing.T) {
		...
		if root.Parent() != nil {
			t.Errorf("Got %v expected nil", root.Parent().Key())
		}
		left := root.Left()
		...
		assert.True(t, left.Parent() == root)
		right := root.Right()
		assert.NotNil(t, right)
		if right.Key() != 5 {
			t.Errorf("Right child should have key = 5")
		}
		assert.True(t, right.Parent() == root)
	})
...
func TestRBTreeSearch(t *testing.T) {
	t1 := redblacktree.New[int, string]()
...
func TestRBTreeRemove2(t *testing.T) {
	t1 := redblacktree.New[int, struct{}]()
...
func TestRBTreeMinAndMax(t *testing.T) {
	t1 := redblacktree.New[int, string]()
...
func TestRBTreeCeilingAndFloor(t *testing.T) {
	t1 := redblacktree.New[int, string]()
...
func TestRBTreeString(t *testing.T) {
	...
	if !strings.HasPrefix(t1.String(), "RedBlackTree") {
...
func TestTraversal(t *testing.T) {
	...
	fmt.Println(values)
	fmt.Println("---")
	keys, values = t2.PostOrderByReverse()
	fmt.Println("PostOrderByReverse:")
	fmt.Println(keys)
	fmt.Println(values)
}
...
func benchmarkSearch(b *testing.B, t1 *redblacktree.Tree[int, struct{}], size int) {
	for b.Loop() {
		for n := range size {
			t1.Search(n)
		}
...
func benchmarkInsert(b *testing.B, t1 *redblacktree.Tree[int, struct{}], size int) {
	for b.Loop() {
		for n := range size {
			t1.Insert(n, struct{}{})
		}
...
func benchmarkRemove(b *testing.B, t1 *redblacktree.Tree[int, struct{}], size int) {
	for b.Loop() {
		for n := range size {
			t1.Remove(n)
		}
