...
// TestHeapInvariants_Randomized stresses the heap with a mix of push, pop,
// remove, fix, and update operations at larger scale than the existing
// TestInitN tests. The verify helper is invoked after every mutation to
// ensure the min-heap property holds throughout.
func TestHeapInvariants_Randomized(t *testing.T) {
	...
	for i := range 5000 {
		...
		case 3:
			...
			_, ok := h.Pop()
			if !ok {
				t.Fatalf("Pop returned !ok with live=%d", live)
			}
			live--
		case 4:
			if live == 0 {
			...
			idx := r.IntN(live)
			h.Update(idx, r.IntN(10000))
		}
		if i%100 == 0 {
			verify(t, h, 0)
			if h.Len() != live {
				t.Fatalf("len mismatch: heap=%d live=%d", h.Len(), live)
			}
		}
	}
	verify(t, h, 0)

	// Drain and verify the output is non-decreasing (min-heap).
	prev, havePrev := 0, false
	for h.Len() > 0 {
		x, ok := h.Pop()
		if !ok {
			t.Fatal("Pop returned !ok on non-empty heap")
		}
		if havePrev && x < prev {
			t.Fatalf("pop order not non-=decreasing: %d then %d", prev, x)
		}
		prev = x
		havePrev = true
		verify(t, h, 0)
...
// TestHeapInvariants_RemoveAnyIndex exercises Remove across arbitrary
// interior indices and ensures the heap invariant is restored.
func TestHeapInvariants_RemoveAnyIndex(t *testing.T) {
	...
	for h.Len() > 0 {
		...
		_, ok := h.Remove(idx)
		if !ok {
			t.Fatalf("Remove(%d) returned !ok with len=%d", idx, h.Len())
		}
		if h.Len()%97 == 0 {
			verify(t, h, 0)
		}
